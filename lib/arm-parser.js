  ...
  _preProcess(resources, parentRes) {
    resources.forEach(res => {
      try {
        ...
        // Resolve and eval resource kind if they are expressions
        if(res.kind){
      ...
      try {
        ...
        if(res.tags) {
          Object.keys(res.tags).forEach(tagname => {
            ...
            tagval = escape(this._evalExpression(tagval));
            tagname = escape(this._evalExpression(tagname));
        ...
        if(res.type == 'microsoft.compute/virtualmachines') {
          ...
          try {
            ...
            if(res.properties.osProfile.computerName) {
              extraData.hostname = escape( this._evalExpression(res.properties.osProfile.computerName) );
            }                              
            if(res.properties.osProfile.adminUsername) {
              extraData.user = escape( this._evalExpression(res.properties.osProfile.adminUsername) ); 
            }
            if(res.properties.hardwareProfile.vmSize) {
              extraData.size = escape( this._evalExpression(res.properties.hardwareProfile.vmSize) ); 
            } 
