class ARMParser {
      ...
      if(res.type == 'microsoft.compute/virtualmachines') {
        ...
        try {
          ...
          if(res.properties.osProfile.computerName) {
            vminfo.hostname = this.evalExpression(res.properties.osProfile.computerName);  
          }                              
          if(res.properties.osProfile.adminUsername) {
            vminfo.user = this.evalExpression(res.properties.osProfile.adminUsername); 
          }
          if(res.properties.hardwareProfile.vmSize) {
            vminfo.size = this.evalExpression(res.properties.hardwareProfile.vmSize); 
          } 
  ...
  funcVariables(varName) {
    ...
    if(findKey) {
      ...
      if(typeof(val) != 'string') return "not-supported";
