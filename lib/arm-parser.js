class ARMParser {
  constructor(templateJSON, paramJSON) {
    this.template = JSON.parse(templateJSON);
    this.elements = [];
    //this.edges = [];
    
    // first pass, fix case and assign ids with a hash function
    ...
    this.template.resources.forEach(res => {
      ...
      // Maybe replace this? Looks mostly OK
      let label = res.type.replace(/microsoft\.\w+\//i, '');
